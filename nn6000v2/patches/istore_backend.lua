...
function index()
    ...
    entry({"linkease", "api"}, call("istore_backend")).leaf=true
end
...
function istore_backend() 
  ...
  local input = {}
  local _uri = http.getenv("REQUEST_URI"):gsub("^/cgi%-bin/luci/linkease/api/", "/cgi-bin/luci/istore/")
  input[#input+1] = http.getenv("REQUEST_METHOD") .. " " .. _uri .. " HTTP/1.1"
